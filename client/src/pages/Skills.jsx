import React, { useEffect, useState } from "react";
import Tilt from "react-parallax-tilt";
import api from "../api/api";

const Skills = () => {
  const [skillsData, setSkillsData] = useState([]);
  const [error, setError] = useState("");

  useEffect(() => {
    async function fetchSkills() {
      try {
        const res = await api.get("/skills");
        setSkillsData(res.data);
      } catch (err) {
        setError("Failed to load skills");
      }
    }
    fetchSkills();
  }, []);

  if (error) return <p className="text-red-500 text-center">{error}</p>;
  if (!skillsData.length) return <p className="text-center text-white">Loading Skills...</p>;

  return (
    <section
      id="skills"
      className="py-24 pb-24 px-[12vw] md:px-[7vw] lg:px-[12vw] font-sans bg-skills-gradient clip-path-custom"
    >
      {/* Section Title */}
      <div className="text-center mb-8">
        <h2 className="text-3xl sm:text-4xl font-bold text-white">SKILLS</h2>
        <div className="w-24 h-1 bg-[#8245ec] mx-auto mt-2"></div>
        <p className="text-gray-400 mt-4 text-lg font-semibold">
          A collection of my technical skills and expertise honed through various projects and experiences
        </p>
      </div>

      {/* Skill Categories */}
      <div className="flex flex-wrap gap-1 lg:gap-5 py-10 justify-between">
        {skillsData.map(({ _id, category, skills }) => (
          <div
            key={_id}
            className="bg-gray-900 backdrop-blur-md px-6 sm:px-6 py-8 sm:py-12 mb-10 w-full sm:w-[45%] rounded-2xl border border-white 
            shadow-[0_0_20px_1px_rgba(130,69,236,0.3)]"
          >
            <h3 className="text-2xl sm:text-3xl font-semibold text-gray-400 mb-4 text-center">
              {category}
            </h3>

            <Tilt
              tiltMaxAngleX={20}
              tiltMaxAngleY={20}
              perspective={1000}
              scale={1.05}
              transitionSpeed={1000}
              gyroscope={true}
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 w-full">
                {skills.map(({ name, logo }, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-center space-x-2 bg-transparent border-2 border-gray-700 rounded-3xl py-2 px-2 sm:py-2 sm:px-3 text-center"
                  >
                    {logo && (
                      <img
                        src={logo}
                        alt={name}
                        className="w-6 h-6 sm:w-8 sm:h-8"
                      />
                    )}
                    <span className="text-xs sm:text-sm text-gray-300">
                      {name}
                    </span>
                  </div>
                ))}
              </div>
            </Tilt>
          </div>
        ))}
      </div>
    </section>
  );
};

export default Skills;
